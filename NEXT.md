...
- 🔄 **IN PROGRESS — Postgres migration (pglite). Full scope mapped + verified.** See `docs/POSTGRES-MIGRATION.md`. Harness `scripts/pg_verify_harness.mjs` ran all 192 against real Postgres: **128 pass unchanged, 13/13 datamarts build with zero changes, 0 semantic mismatches; 64 need mechanical SQLite-ism ports** (round→::numeric ~30, strftime→to_char/EXTRACT ~18, julianday→date-subtract ~14, date()→interval 2, instr→strpos 1, alias-in-HAVING 1). **MUST ship atomically** (port + engine swap in ONE commit — porting alone breaks the live SQLite engine). NEXT: port the 64 + all brokenQuery + methods.sql → harness 192/192 → swap sql.js→pglite (async runner + validateResults) → update spec/CLAUDE/cheatsheet → push once. `@electric-sql/pglite` confirmed working in-sandbox.
