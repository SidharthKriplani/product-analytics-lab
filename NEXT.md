...
**3. SQL coverage gaps (re-scoped 2026-06-22 by SQL-COVERAGE-REPORT.md)** — audit vs SQL-VARIETY-BENCHMARK found the real holes are: **sessionization (0 — P1)**, **true median without builtin (0; only PERCENT_RANK exists — P2)**, **gaps-and-islands (1, thin — P3)**, set ops (1, thin), recursive (2, thin). The old AUDITS.md #132 list is partly stale — ROWS BETWEEN, PERCENT_RANK, and recursive CTE now exist. Still-missing from #132: date-spine/gap-filling (overlaps P3) + two-valid-queries format. See `docs/SQL-COVERAGE-REPORT.md` for 11 propose-only specs (SESS-1/2, MED-1/2, GAPS-2/3, SET-1, REC-1, STR-1, DEDUP-1) + suggested build order. Awaiting Sidharth approval to build. Run audit script clean before pushing.
