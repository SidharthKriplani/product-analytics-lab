...
- ✅ **QUEUED — Postgres flavor migration (BIG).** Move SQL Lab off sql.js (SQLite) to **pglite** (Postgres-in-WASM). Scope: swap engine; port datamart types + every SQLite-ism in solutions (strftime/julianday/date-modifier → Postgres EXTRACT/interval/casts); regenerate expected outputs under Postgres; new Postgres verify harness. Also aligns the cheat sheet (already Postgres-syntax) with the engine. Dedicated multi-session effort.
