export const fullLoopCases = [
  {
    ...
    phases: [
      ...
      },
      {
        ...
        fields: [
          { label: 'Hypothesis', correctAnswer: 'Switching to the fixed UPI integration will restore checkout completion rate for UPI users to pre-migration levels (above 60%).' },
          { label: 'Unit of randomization', correctAnswer: 'User-level randomization. Each user is assigned to either the old fallback provider (control) or the new fixed provider (treatment) for all their UPI transactions.' },
          { label: 'Primary metric', correctAnswer: 'UPI payment completion rate (completed UPI payments / attempted UPI payments).' },
          { label: 'Guardrail metrics', correctAnswer: 'Payment error rate, average transaction processing time, customer support ticket volume related to payments, and refund rate.' },
        ],
      },
      {
        type: 'readout',
  ...
  },
  {
    ...
    phases: [
      ...
      },
      {
        ...
        fields: [
          { label: 'Hypothesis', correctAnswer: 'Showing a push notification re-enablement prompt to iOS users who lost push registration will recover their notification delivery rate and increase their DAU retention back to pre-bug levels.' },
          { label: 'Unit of randomization', correctAnswer: 'User-level. Each affected iOS user is randomly assigned to see the re-enablement prompt (treatment) or not (control).' },
          { label: 'Primary metric', correctAnswer: 'D7 retention rate for affected iOS users (percentage of users active on day 7 after the prompt).' },
          { label: 'Guardrail metrics', correctAnswer: 'Push notification opt-out rate (ensure the prompt does not cause more users to actively disable notifications), app uninstall rate, and session count per user.' },
        ],
      },
      {
        type: 'readout',
  ...
  },
  {
    ...
    phases: [
      ...
      },
      {
        ...
        fields: [
          { label: 'Hypothesis', correctAnswer: 'The new ranking model increases GMV per search session at the cost of reduced CTR and satisfaction on tail queries. Applying it only to head queries will capture most of the revenue upside while preserving tail query experience.' },
          { label: 'Unit of randomization', correctAnswer: 'Search-session level, stratified by query type (head vs tail). Each search session is independently assigned to old or new ranking model.' },
          { label: 'Primary metric', correctAnswer: 'GMV per search session (to capture revenue impact) and search CTR (to capture user satisfaction impact). This is a dual-primary metric test.' },
          { label: 'Guardrail metrics', correctAnswer: 'Tail-query CTR specifically, search abandonment rate (searches with zero clicks), repeat search rate within 24 hours (indicates user dissatisfaction), and return rate on purchased items.' },
        ],
      },
      {
        type: 'readout',
  ...
  },
  {
    ...
    phases: [
      ...
      },
      {
        ...
        fields: [
          { label: 'Hypothesis', correctAnswer: 'Offering a 3-month price lock at the old monthly rate to annual-to-monthly switchers will reduce their churn rate from 22% to below 8% by easing the price transition, without significantly reducing per-user revenue over 6 months.' },
          { label: 'Unit of randomization', correctAnswer: 'User-level. Each annual-to-monthly switcher is randomly assigned to receive the price lock offer (treatment) or the standard new price (control).' },
          { label: 'Primary metric', correctAnswer: 'Monthly churn rate for the annual-to-monthly switcher cohort, measured at 30, 60, and 90 days post-switch.' },
          { label: 'Guardrail metrics', correctAnswer: 'Revenue per user at 6-month horizon (to ensure the discount pays for itself through retention), annual plan downgrade rate (to ensure the offer does not incentivize gaming by switching plans to get the discount), and support ticket volume.' },
        ],
      },
      {
        type: 'readout',
  ...
  },
  {
    ...
    phases: [
      ...
      },
      {
        ...
        fields: [
          { label: 'Hypothesis', correctAnswer: 'Capping discounts at 15% for Tier 2/3 cities with more than 20% historical RTO rate will improve contribution margin per order by reducing both discount cost and the volume of discount-driven orders that get returned, while maintaining order volume from genuine buyers.' },
          { label: 'Unit of randomization', correctAnswer: 'City-level randomization. Each eligible city is assigned to treatment (15% cap) or control (uncapped) to avoid within-city spillover effects from users sharing discount codes.' },
          { label: 'Primary metric', correctAnswer: 'Contribution margin per order in the treated cities, calculated as revenue minus discount minus logistics minus RTO cost.' },
          { label: 'Guardrail metrics', correctAnswer: 'Total order volume (acceptable to decline up to 10%), new user acquisition rate, RTO rate (should improve with less discount-driven buying), customer satisfaction scores, and repeat purchase rate for non-discount orders.' },
        ],
      },
      {
        type: 'readout',
