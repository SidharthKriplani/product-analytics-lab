...
FROM stats` },
...

  {
    ...
    category: 'BI',
    title: 'Dashboard Design That Actually Gets Used',
    summary: 'Most dashboards answer questions nobody asked. Good ones are built backward from a decision.',
    readMin: 7,
    source: null,
    relatedItems: [
      { label: 'BI Room — BI01: The Dashboard That Lies', room: 'bi', id: 'BI01' },
      { label: 'BI Room — BI04: The Self-Serving Dashboard', room: 'bi', id: 'BI04' },
    ],
    content: [
      { type: 'text', text: 'Most dashboards fail not because the data is wrong, but because they were designed to show everything rather than to drive a decision. A dashboard that gets used was built by starting with one question: what decision does this person need to make, and what is the minimum data required to make it?' },
      { type: 'heading', text: 'The Audience-First Problem' },
      { type: 'text', text: 'A single dashboard serving an executive, an analyst, and a growth manager is a dashboard that serves none of them well. The executive needs a signal — is the business healthy or not? The analyst needs diagnostics — where exactly is the problem? The growth manager needs action — which experiment should I run next?\n\nThese are different questions at different latency and granularity requirements. An executive dashboard that shows 47 metrics is providing cover, not clarity. A diagnostic dashboard that shows only top-line KPIs is analytically useless. Separate the audience before you design the layout.' },
      { type: 'heading', text: 'The Vanity Metric Trap' },
      { type: 'text', text: 'Vanity metrics are metrics that trend up reliably regardless of whether the business is actually improving. Total registered users, total messages sent, total sessions — these accumulate by default. They feel good because they almost always go up. They are useless for decision-making because they cannot go down in a way that tells you something is wrong.\n\nThe test for a vanity metric: can this number increase while the business gets worse? If yes, it\'s vanity. Total registered users can grow while daily active users collapse. Total messages can grow while per-user engagement falls. Include vanity metrics on dashboards only as context, never as primary signals.' },
      { type: 'callout', label: 'The decision test', text: 'Before adding any metric to a dashboard, ask: if this number changed significantly tomorrow, would it change what someone does? If the answer is no — if it\'s informational but not actionable — it belongs in an appendix, not on the main view.' },
      { type: 'heading', text: 'Signal-to-Noise Ratio in Dashboard Layout' },
      { type: 'list', items: [
        'Primary signal (top left, largest): the one number that summarizes health. DAU, revenue, conversion rate — whatever the business runs on.',
        'Diagnostic layer (below primary): 3–5 metrics that explain movements in the primary signal. If DAU dropped, which segment? Which channel? Which cohort?',
        'Guardrail layer (right column or footer): metrics that should not move even when we are optimizing the primary signal. Latency, error rate, support ticket volume, refund rate.',
        'Avoid the everything approach: a dashboard with 30 metrics of equal visual weight forces the viewer to decide what matters. That is your job as the designer, not theirs.',
      ]},
      { type: 'heading', text: 'The Self-Serving Dashboard Anti-Pattern' },
      { type: 'text', text: 'A self-serving dashboard is one designed to make a team, product, or decision look good rather than to reflect reality. This shows up as: selecting metrics where the team performed well while omitting those where they did not; using a time window that happened to start just before a bad period and end just after a good one; or displaying percentage improvements when the absolute numbers are small.\n\nThis is a trust problem. Once stakeholders notice that your dashboards reliably tell a positive story, they stop trusting them. The most persuasive dashboard is the one that shows both the wins and the problems honestly — because that one is believed.' },
      { type: 'cta', room: 'bi', label: 'Practice BI Room →' },
    ],
    keyTakeaways: [
      'Build dashboards backward from a decision, not forward from available data.',
      'Separate audiences: executives need signal, analysts need diagnostics, operators need action triggers.',
      'Vanity metric test: can this number increase while the business gets worse? If yes, demote it from primary signal.',
      'Layout hierarchy: primary signal → diagnostic layer → guardrail layer. Avoid 30 metrics of equal visual weight.',
      'A self-serving dashboard that cherry-picks positive metrics destroys stakeholder trust faster than any bad result.',
    ],
  },

  {
    id: 'bi-attribution-models',
    category: 'BI',
    title: 'Attribution Models: Why Your Channel Credits Don\'t Add Up',
    summary: 'Last-touch attribution systematically lies to you about which channels drive value. Here\'s the full picture.',
    readMin: 8,
    source: null,
    relatedItems: [
      { label: 'BI Room — BI03: The Attribution Dispute', room: 'bi', id: 'BI03' },
    ],
    content: [
      { type: 'text', text: 'Attribution is the problem of assigning credit for a conversion to the marketing touchpoints that preceded it. It sounds like an accounting question, but it is actually a product question: which channels are genuinely driving value, and which are merely present at the moment of conversion? Getting this wrong sends budget to the wrong place.' },
      { type: 'heading', text: 'The Five Attribution Models' },
      { type: 'list', items: [
        'Last-touch: 100% of credit to the final touchpoint before conversion. Simple, widely used, and systematically wrong for multi-channel journeys. Over-credits retargeting and search — channels that are present at the end but did not generate the intent.',
        'First-touch: 100% credit to the first touchpoint. Over-credits awareness channels (social, display) that started the journey but may not have been decisive.',
        'Linear: equal credit split across all touchpoints in the journey. Defensible but blunt — treats a brand awareness impression and a high-intent search click as equally valuable.',
        'Time-decay: more credit to touchpoints closer to conversion. The implicit assumption is that recent interactions were more causally important. Reasonable for short sales cycles.',
        'Data-driven (algorithmic): uses historical conversion data to estimate the marginal contribution of each touchpoint via counterfactual reasoning. Most accurate but requires volume (typically 1,000+ conversions per channel per week) and is a black box to non-technical stakeholders.',
      ]},
      { type: 'callout', label: 'The core problem with last-touch', text: 'A user sees a YouTube pre-roll ad on Monday, reads a blog post Thursday, searches for the product by name Friday, and converts via a branded search click. Last-touch gives 100% credit to branded search. But branded search was the measurement of conversion intent, not the cause of it. The YouTube ad and blog post created the intent.' },
      { type: 'heading', text: 'The Attribution Dispute: Channels Claiming Credit' },
      { type: 'text', text: 'In a multi-channel environment, if you sum up each channel\'s reported conversions, you will almost always exceed total actual conversions. This is because most attribution platforms default to last-touch, and if a user touched both Facebook and Google before converting, both channels may count that conversion. This creates political fights: the Facebook team says they drove 40,000 conversions; the Google team says they drove 38,000; total actual conversions were 45,000.\n\nThe fix is to choose a single attribution methodology applied consistently across all channels, and to report attribution-adjusted numbers rather than each channel\'s self-reported numbers.' },
      { type: 'heading', text: 'Incrementality Testing: The Only Ground Truth' },
      { type: 'text', text: 'All attribution models are estimates, not measurements. The only way to measure the true incremental impact of a channel is an incrementality test: randomly withhold the channel from a control group and measure the conversion rate difference. If removing Facebook ads from 10% of your audience drops their conversion rate by 2.1%, Facebook is genuinely driving ~2.1pp of conversions — regardless of what any attribution model says.\n\nIncrementality tests are expensive (you are deliberately not running ads to some users) and require statistical power. But they are the calibration tool that tells you whether your attribution model is biased.' },
      { type: 'heading', text: 'Applied Example: DTC Brand Attribution Audit' },
      { type: 'example', label: 'Attribution Audit — Presto Direct-to-Consumer Brand', text: 'SCENARIO\n  Presto DTC skincare brand\n  Monthly budget: $180K across paid social, SEM, email, influencer\n  Problem: channels\' self-reported conversions sum to 87K\n  Actual monthly conversions: 34K\n  Attribution model in use: last-touch per-channel (different vendors)\n\nCHANNEL SELF-REPORTED vs LAST-TOUCH ADJUSTED\n\n  Channel         Self-reported    Last-touch      Difference\n  Paid Social         31,000          8,200           -22,800 (73%)\n  SEM (branded)       28,000         14,600           -13,400 (48%)\n  SEM (non-brand)      9,000          5,100            -3,900 (43%)\n  Email               12,000          4,800            -7,200 (60%)\n  Influencer           7,000          1,300            -5,700 (81%)\n  ─────────────────────────────────────────────────────────────────\n  Sum                 87,000         34,000           -53,000\n\nOBSERVATION\n  Paid Social and Influencer are most over-inflated (73%, 81%)\n  SEM Branded is likely capturing intent created by other channels\n  Email shows moderate overlap — strong D1 sends likely attributing\n  a conversion that was already decided\n\nRECOMMENDATION\n  Run incrementality holdout for Paid Social and Influencer\n  Use time-decay model across all channels as interim standard\n  Reduce Influencer budget until incrementality is confirmed\n  Flag SEM Branded as a conversion measurement channel, not acquisition' },
      { type: 'cta', room: 'bi', label: 'Practice BI Room →' },
    ],
    keyTakeaways: [
      'Last-touch attribution over-credits bottom-funnel channels (branded search, retargeting) that measure intent rather than create it.',
      'When channels\' self-reported conversions sum to more than total actual conversions, you have multi-touch overlap — usually because each channel uses last-touch independently.',
      'Data-driven attribution is most accurate but requires high conversion volume and is a black box to stakeholders.',
      'Incrementality testing (holdout experiments) is the only ground truth for whether removing a channel actually reduces conversions.',
      'In the Presto example: 87K self-reported conversions vs 34K actual — Paid Social and Influencer were most inflated (73% and 81% over-claimed).',
    ],
  },

  {
    id: 'bi-seasonality-vs-trend',
    category: 'BI',
    title: 'Separating Seasonality from Real Trend',
    summary: 'A metric drop that repeats every year is not an alarm. A metric drop that never happened before is.',
    readMin: 6,
    source: null,
    relatedItems: [
      { label: 'BI Room — BI08: Seasonality vs Real Trend', room: 'bi', id: 'BI08' },
    ],
    content: [
      { type: 'text', text: 'When a metric drops, the first question is not "what caused this?" but "is this real?" A 15% drop in weekly revenue every January is not a crisis — it is expected seasonal behavior. But a 15% drop in March, when March is normally flat, is signal that demands investigation. Confusing seasonal variation with trend is one of the most common and costly BI errors.' },
      { type: 'heading', text: 'The Three Comparison Types' },
      { type: 'list', items: [
        'WoW (Week-over-Week): sensitive to noise, captures immediate changes, heavily distorted by day-of-week effects and short-term seasonality. Good for ops monitoring, bad for strategic decisions.',
        'MoM (Month-over-Month): smoother than WoW but still affected by month-length variation, holiday calendars, and promotional timing. Appropriate for tactical reporting.',
        'YoY (Year-over-Year): the gold standard for separating seasonality from trend. Compares this January to last January, this Q3 to last Q3. Removes the seasonal component entirely. The weakness: it is slow — you need 12 months of history, and you are always comparing to 12 months ago.',
      ]},
      { type: 'callout', label: 'The YoY default rule', text: 'For any metric with known seasonality — e-commerce revenue, travel bookings, subscription signups with back-to-school cycles — always anchor analysis to YoY comparisons. A team that panics every January because WoW revenue dropped is not doing analysis; it is pattern-matching to noise.' },
      { type: 'heading', text: 'Time-Series Decomposition' },
      { type: 'text', text: 'Formal decomposition breaks a time series into three components:\n\nTrend: the long-run direction of the metric, smoothed of seasonal and random variation.\n\nSeasonal: the repeating cyclical pattern (weekly, monthly, annual) that is predictable from history.\n\nResidual (noise): the unexplained variation after removing trend and seasonal components.\n\nWhen a metric drops, the question is: which component changed? If the residual spiked but the trend held, the drop is probably noise. If the seasonal component is normal but trend has been declining for 6 weeks, the drop is structural. If the seasonal component is lower than last year\'s seasonal component at the same point, something has shifted in user behavior.' },
      { type: 'heading', text: 'Applied Example: EdTech Platform Revenue Anomaly' },
      { type: 'example', label: 'Seasonality Analysis — Luminos EdTech Platform', text: 'SCENARIO\n  Luminos online education platform\n  Revenue dropped 22% in week of July 8 vs prior week\n  Leadership alarmed — requesting urgent investigation\n\nSTEP 1: CHECK PRIOR YEAR\n  Week of July 9 (prior year): -18% vs prior week\n  Week of July 10 (two years prior): -21% vs prior week\n  → July dip is a recurring annual pattern (summer holiday onset)\n  → This year\'s drop is within 4pp of prior year pattern\n\nSTEP 2: YoY COMPARISON\n  This July 8 week revenue: $2.84M\n  Prior year July 9 week revenue: $2.71M\n  YoY growth: +4.8%\n  → On a YoY basis, this week is GROWING. No anomaly.\n\nSTEP 3: RESIDUAL CHECK\n  After removing seasonal component:\n  Trend line: +3.2% YoY (consistent with last 6 months)\n  Residual: +1.4% (above trend, within noise)\n  → No structural change detected\n\nCONCLUSION\n  The -22% WoW drop is entirely seasonal.\n  On YoY basis, the business is growing normally.\n  Recommendation: set automatic seasonal annotations on WoW\n  dashboards to prevent future false alarms.' },
      { type: 'cta', room: 'bi', label: 'Practice BI Room →' },
    ],
    keyTakeaways: [
      'The first question when a metric drops is not "what caused this?" but "is this real?" — check whether the pattern repeats annually.',
      'WoW comparisons are most sensitive to seasonality distortion. YoY is the standard for any metric with known cyclicality.',
      'Time-series decomposition isolates trend, seasonal, and residual components — only the residual and trend components are signal.',
      'In the Luminos example: a 22% WoW drop was entirely seasonal (matched prior two years); on a YoY basis, revenue was +4.8%.',
      'Set seasonal annotations on WoW dashboards to prevent recurring false alarms from known cyclical patterns.',
    ],
  },

  {
    id: 'bi-data-storytelling',
    category: 'BI',
    title: 'Data Storytelling for Stakeholders: Findings Before Numbers',
    summary: 'The most common mistake in data presentations: leading with the data. Lead with the finding.',
    readMin: 6,
    source: null,
    relatedItems: [
      { label: 'BI Room — BI07: Board-Level Data Storytelling', room: 'bi', id: 'BI07' },
    ],
    content: [
      { type: 'text', text: 'A data presentation that makes the audience read the chart to figure out what happened is a failed presentation. The analyst\'s job is to do the work of interpretation so the audience does not have to. That means leading with the finding, not the methodology, the data quality caveats, or the slide title "Q3 Revenue Analysis."' },
      { type: 'heading', text: 'The Pyramid Principle Applied to Data' },
      { type: 'text', text: 'The Pyramid Principle (Barbara Minto, McKinsey) states that communication should start with the conclusion and then provide the supporting logic. For data storytelling, this translates to:\n\nSlide 1 title: "Checkout drop-off increased 8pp in Q3, driven by mobile users on iOS 17" — not "Checkout Funnel Analysis Q3."\n\nSlide 2: here is the funnel data that proves it.\n\nSlide 3: here is the iOS 17 breakout.\n\nSlide 4: here is the recommendation.\n\nMost analysts invert this. They show the data first, then build toward the conclusion. The audience is left doing the interpretation work themselves, and if they reach a different conclusion than you did, you have lost the room before you got to the recommendation.' },
      { type: 'callout', label: 'The slide title test', text: 'Replace every slide title with "So what?" If your title already answers "so what?" — e.g., "iOS 17 users convert 8pp less than Android" — it is a finding title. If it just describes the content — "Conversion by OS" — it is a label title. A deck full of label titles is a data dump. A deck full of finding titles is a story.' },
      { type: 'heading', text: 'The SCR Framework for Opening Any Analysis' },
      { type: 'list', items: [
        'Situation: the context the audience already knows. Keep this one sentence. "We have been investigating why Q3 checkout conversion dropped."',
        'Complication: what changed or what problem you found. "We found that the drop is entirely concentrated in iOS 17 users, who convert 8pp lower than last quarter."',
        'Resolution: what you recommend or what action this implies. "The recommended fix is to patch the payment sheet modal that renders incorrectly on iOS 17 — estimated 3-day engineering effort, projected to recover $1.2M in monthly revenue."',
      ]},
      { type: 'heading', text: 'Handling Uncertainty Without Undermining Confidence' },
      { type: 'text', text: 'A common failure mode in data presentations is burying the finding in caveats. "Although the data has some quality issues, and we couldn\'t fully control for the marketing campaign that ran in the same period, and the sample size for mobile is smaller than ideal, it appears that iOS 17 may have had some impact..." — by the time you reach the conclusion, the audience has lost confidence in the analysis.\n\nThe alternative is to separate finding from caveat. State the finding clearly first. Then note the limitation. "iOS 17 drove the Q3 conversion drop. One caveat: we cannot fully isolate this from the concurrent campaign — the campaign ran equally across OS versions, which supports the attribution, but I\'d recommend confirming with an iOS 17 targeted holdout before committing the full engineering fix."' },
      { type: 'cta', room: 'bi', label: 'Practice BI Room →' },
    ],
    keyTakeaways: [
      'Lead with the finding, not the data. "Checkout conversion dropped 8pp on iOS 17" as the title, not "Checkout Funnel Analysis Q3."',
      'The Pyramid Principle: conclusion first, supporting logic second. Most analysts invert this and lose the audience before the recommendation.',
      'Slide title test: if a title just describes the content ("Conversion by OS"), it is a label. If it states the finding ("iOS 17 users convert 8pp less"), it is a story.',
      'SCR framework: Situation (one sentence of context) → Complication (what you found) → Resolution (what you recommend).',
      'Separate findings from caveats. State the finding clearly first, then note the limitation — do not bury the conclusion in uncertainty hedges.',
    ],
  },

  {
    id: 'instrumentation-measurement-plan',
    category: 'Instrumentation',
    title: 'Measurement Plans: Write the Questions Before the Events',
    summary: 'Instrumentation designed without a measurement plan answers questions nobody asked.',
    readMin: 7,
    source: null,
    relatedItems: [
      { label: 'Instrumentation Room — inst01: Measurement Plan for Checkout Redesign', room: 'instrumentation', id: 'inst01' },
      { label: 'Instrumentation Room — inst07: Tracking Plan for a New Feature Launch', room: 'instrumentation', id: 'inst07' },
    ],
    content: [
      { type: 'text', text: 'A measurement plan is a document that specifies what questions the team needs to answer about a feature, what metrics will answer each question, and what events and properties are required to compute those metrics. It is written before any tracking code is shipped. Teams that skip the measurement plan end up with events that do not quite capture what they needed, missing properties, and gaps they only discover when the feature is already in production.' },
      { type: 'heading', text: 'The Question → Metric → Event Chain' },
      { type: 'text', text: 'Every event in a tracking plan should trace back to a business question. The chain is:\n\nQuestion: what do we want to know? ("Are users completing the checkout redesign at a higher rate?")\n\nMetric: what number answers that question? (Checkout completion rate = checkout_completed events / checkout_started events)\n\nEvent: what action must the user take for that event to fire? (checkout_started fires when the first step of checkout loads; checkout_completed fires when the order confirmation page loads)\n\nProperties: what context do we need to attach to each event? (user_id, session_id, checkout_flow_variant, item_count, cart_value, payment_method, platform)\n\nIf you cannot trace an event back to a question, you probably do not need that event.' },
      { type: 'callout', label: 'The "so what" filter', text: 'For every proposed event, ask: if this event fired at twice the rate it does today, what decision would we make differently? If the answer is "nothing" — if the event is interesting but not decision-relevant — cut it from V1. Tracking everything is a data quality and cost problem.' },
      { type: 'heading', text: 'Critical Events vs Nice-to-Haves' },
      { type: 'list', items: [
        'Critical events: events whose absence makes it impossible to evaluate the feature. For a checkout redesign: checkout_started, checkout_step_viewed, checkout_completed, checkout_abandoned. These must ship with the feature.',
        'Diagnostic events: events that help explain why the critical metrics moved. Step-level drop-off events, error events, help interactions. High value, can be added in V1.1 if time-constrained.',
        'Nice-to-have events: granular interactions (hover time, scroll depth, tooltip views) that are interesting but not tied to a decision. Defer or cut.',
      ]},
      { type: 'heading', text: 'Common Measurement Plan Failures' },
      { type: 'text', text: 'Missing denominator events: a team instruments checkout_completed but not checkout_started. They can count completions but cannot compute a completion rate because the denominator was never tracked.\n\nInconsistent naming: one team sends add_to_cart, another sends cart_item_added, a third sends product_added. All three fire for the same action. Downstream, analysts cannot aggregate across surfaces.\n\nMissing properties at the wrong moment: a purchase event fires without the experiment variant property. Months later, when analyzing an A/B test\'s impact on revenue, the variant is unrecoverable.\n\nTiming assumption errors: a feature_viewed event fires when a component mounts in the DOM, not when the user actually scrolls it into the viewport. The view rate is inflated by ~40%.' },
      { type: 'cta', room: 'instrumentation', label: 'Practice Instrumentation Room →' },
    ],
    keyTakeaways: [
      'A measurement plan specifies questions → metrics → events → properties, written before tracking code ships.',
      'Every event must trace back to a business question. If you cannot state what decision the event informs, cut it from V1.',
      'Prioritize: critical events (completion rate numerator AND denominator) ship with the feature; diagnostic and nice-to-have events follow.',
      'Common failures: missing denominator events, inconsistent naming across surfaces, missing properties at fire time, timing assumption errors.',
      'The "so what" filter: if this event doubled, would any decision change? If no, defer it.',
    ],
  },

  {
    id: 'instrumentation-event-taxonomy',
    category: 'Instrumentation',
    title: 'Event Taxonomy Design: The Naming Decisions That Save You Later',
    summary: 'Event names chosen carelessly create years of analytical debt. Here\'s the standard that prevents it.',
    readMin: 7,
    source: null,
    relatedItems: [
      { label: 'Instrumentation Room — inst02: Event Taxonomy Design', room: 'instrumentation', id: 'inst02' },
    ],
    content: [
      { type: 'text', text: 'Event taxonomy is the naming and structural convention for all events in a tracking system. It seems like a housekeeping decision — snake_case vs camelCase, how verbose to make names — but it compounds over time. A taxonomy chosen without a standard becomes analytically unmaintainable at 500+ event types, where 40% of events have duplicate purpose and 30% have inconsistent property schemas.' },
      { type: 'heading', text: 'The Entity-Action-Object Pattern' },
      { type: 'text', text: 'The most durable event naming convention is the Entity-Action-Object (or Object-Action) pattern:\n\nEntity (optional): the surface or product area. "checkout", "onboarding", "search"\n\nObject: the thing being acted on. "product", "form", "button", "page"\n\nAction: what happened to it. "viewed", "clicked", "submitted", "completed", "abandoned"\n\nExamples:\n  checkout_page_viewed\n  search_result_clicked\n  onboarding_form_submitted\n  payment_method_selected\n  account_created\n\nThe pattern enforces parallel structure: all views are "[thing]_viewed", all clicks are "[thing]_clicked". Analysts can search for all view events with a wildcard. Downstream query patterns become predictable.' },
      { type: 'callout', label: 'The verb tense standard', text: 'Use past tense (viewed, clicked, submitted) for events — they record what happened. Use present tense only for property values. "event_name: checkout_page_viewed" not "checkout_page_view". This matters when reading event streams: "a checkout_page_viewed event fired" parses naturally; "a checkout_page_view event fired" does not.' },
      { type: 'heading', text: 'Property Schema Standards' },
      { type: 'list', items: [
        'Universal properties (on every event): user_id or anonymous_id, session_id, timestamp, platform (web/ios/android), app_version, page_url or screen_name.',
        'Entity properties (on relevant events): item_id, category_id, experiment_variant — any identifier that lets you join the event to other tables.',
        'Type consistency: if item_price is a float on one event and a formatted string ("$24.99") on another, you cannot aggregate across them. Define types per property name, not per event.',
        'Null vs absent: decide whether a missing property sends null or is omitted entirely. Absent properties make schema validation harder; null properties bloat storage. Pick one convention and enforce it.',
      ]},
      { type: 'heading', text: 'Versioning and Deprecation' },
      { type: 'text', text: 'Event schemas change. A property gets renamed, a new required field is added, a value enum expands. Without versioning, schema changes break downstream queries silently.\n\nThe standard pattern:\n  - Track a schema_version property on events, or\n  - Create a new event name (checkout_completed_v2) when the schema changes meaningfully\n  - Maintain a data contract doc that specifies current schema per event\n  - Keep the old event firing in parallel for 30 days before sunsetting\n\nSilent schema breaks — where the event still fires but a property changes meaning — are the most dangerous failure mode in analytics pipelines.' },
      { type: 'cta', room: 'instrumentation', label: 'Practice Instrumentation Room →' },
    ],
    keyTakeaways: [
      'Entity-Action-Object naming (checkout_page_viewed, search_result_clicked) creates parallel structure that makes event streams queryable and predictable.',
      'Use past tense for event names (viewed, clicked, submitted) — events record what happened.',
      'Universal properties ship on every event: user_id, session_id, timestamp, platform, app_version.',
      'Type consistency matters: item_price must be the same type (float, not mixed) across all events for aggregation to work.',
      'Silent schema breaks — property renamed or meaning changed without versioning — are the most dangerous analytics pipeline failure.',
    ],
  },

  {
    id: 'instrumentation-data-quality',
    category: 'Instrumentation',
    title: 'Diagnosing Data Quality Incidents: The Isolation Methodology',
    summary: 'When event volume drops 40% overnight, the question is not "what broke?" but "where exactly did it break?"',
    readMin: 8,
    source: null,
    relatedItems: [
      { label: 'Instrumentation Room — inst03: Data Quality Incident Investigation', room: 'instrumentation', id: 'inst03' },
      { label: 'Instrumentation Room — inst08: Instrumentation Debt Audit', room: 'instrumentation', id: 'inst08' },
    ],
    content: [
      { type: 'text', text: 'A data quality incident is any event where analytics data deviates from reality: event volume drops, properties populate incorrectly, a new property appears on some events but not others, or a metric in the dashboard does not match what operations teams observe on the ground. These incidents range from a misconfigured tag manager rule to a fundamental tracking break that corrupts weeks of A/B test data.' },
      { type: 'heading', text: 'Three Types of Data Quality Problems' },
      { type: 'list', items: [
        'Sudden drop or spike: event volume changes abruptly. Usually caused by a code deployment (tracking code removed or broken), infrastructure issue (event pipeline down), or configuration change (tag manager rule change, consent banner change). Start by correlating with deployment and release timestamps.',
        'Gradual drift: event volume or property values drift over weeks. Usually caused by behavioral change (real), a slow rollout affecting tracking (new app version gradually replacing old), or a data pipeline transformation that introduced rounding or filtering. Harder to detect; requires baseline comparison over a rolling window.',
        'Schema break: events fire but properties are wrong — null where they should be populated, wrong types, incorrect values. Usually caused by a frontend refactor that changed variable names the tracking code referenced, or a backend event that changed its payload schema.',
      ]},
      { type: 'heading', text: 'The Isolation Methodology' },
      { type: 'text', text: 'When investigating a data quality incident, isolate the scope before guessing the cause. The isolation dimensions are:\n\nWhen: what is the exact timestamp of the break? Align with deployment history.\n\nWhere: which surfaces are affected? Web only? iOS only? A specific page or feature?\n\nWhat: which events are affected? All events from a surface, or specific event types?\n\nWho: which users are affected? All users, or a specific segment (new users, users on a specific app version, users who went through a specific funnel step)?\n\nNarrowing these four dimensions tells you where to look in the code. If only iOS 17.2 users on the checkout flow lost purchase events starting Wednesday at 14:30 UTC, you look at the iOS 17.2 deployment notes and the checkout payment confirmation code.' },
      { type: 'callout', label: 'The deployment correlation rule', text: 'In 80% of sudden data quality incidents, the root cause is a code or configuration change that went out in the prior 24 hours. Before doing anything else, pull the deployment log and align the break timestamp with releases. This eliminates most hypotheses in 10 minutes.' },
      { type: 'heading', text: 'Data Quality in A/B Tests: The Most Dangerous Case' },
      { type: 'text', text: 'Data quality incidents that occur during an active A/B test are the highest-severity variant. If a tracking break affects the control group differently from the treatment group, every metric in the experiment is contaminated. The standard check is Sample Ratio Mismatch (SRM): the actual ratio of users assigned to each variant should match the configured ratio (e.g., 50/50). If you configured 50/50 but see 63/37 in the data, either assignment is broken or tracking is broken — either way, the experiment is invalid.\n\nSRM detection should run as an automated check every 24 hours during any live experiment. An SRM p-value below 0.001 should automatically flag the experiment for investigation.' },
      { type: 'cta', room: 'instrumentation', label: 'Practice Instrumentation Room →' },
    ],
    keyTakeaways: [
      'Three incident types: sudden drop (deployment/config change), gradual drift (rollout or pipeline transformation), schema break (frontend refactor or payload change).',
      'Isolation methodology: narrow When, Where, What, and Who before guessing the cause.',
      'In 80% of sudden incidents, a deployment in the prior 24 hours is the root cause. Correlate the break timestamp with the release log first.',
      'Schema breaks are the most insidious: events fire normally, but property values are wrong — only caught by comparing against a known baseline.',
      'Sample Ratio Mismatch (SRM) during an A/B test is a data quality incident. An SRM p < 0.001 means the experiment data is invalid and should not be analyzed.',
    ],
  },

  {
    id: 'instrumentation-ab-tracking',
    category: 'Instrumentation',
    title: 'A/B Test Instrumentation: What Goes Wrong and How to Prevent It',
    summary: 'Most A/B test failures are instrumentation failures. The experiment logic is usually fine.',
    readMin: 7,
    source: null,
    relatedItems: [
      { label: 'Instrumentation Room — inst04: A/B Test Instrumentation Audit', room: 'instrumentation', id: 'inst04' },
      { label: 'Instrumentation Room — inst05: Privacy-Compliant Event Design', room: 'instrumentation', id: 'inst05' },
    ],
    content: [
      { type: 'text', text: 'An A/B test can have a valid hypothesis, correct statistical design, and sufficient power, and still produce uninterpretable results because the instrumentation was wrong. Instrumentation failures in experiments are common, subtle, and often not discovered until analysis — at which point the experiment has already run for weeks and the window to collect clean data has closed.' },
      { type: 'heading', text: 'Assignment vs Exposure Events' },
      { type: 'text', text: 'The critical distinction in experiment instrumentation is between assignment and exposure:\n\nAssignment: the moment a user is assigned to a variant by the experiment system. This happens server-side or in the SDK when the feature flag is evaluated. Every user in the experiment has an assignment event.\n\nExposure: the moment a user actually encounters the treatment — they view the component, see the changed UI, or reach the modified step. This is a subset of assigned users.\n\nThe mistake is using assignment as the denominator for metric calculations. If 50,000 users were assigned to the experiment but only 30,000 reached the page where the change was visible, using 50,000 as the denominator dilutes the measured effect by 40%. Always analyze on exposed users, not assigned users.\n\nAnd crucially: the exposure event must fire for both control and treatment. If it only fires when a user sees the new UI (treatment), you have no control baseline.' },
      { type: 'callout', label: 'The exposure event rule', text: 'Ship an explicit experiment_exposed event (or feature_flag_exposure) that fires when a user actually encounters the experimental treatment — for both control and treatment groups. This is the cleanest denominator for all metric calculations and enables post-hoc power analysis on the actual exposed population.' },
      { type: 'heading', text: 'The Pre-Experiment Instrumentation Checklist' },
      { type: 'list', items: [
        'Assignment event: fires server-side or in the SDK at variant assignment. Includes user_id, experiment_id, variant_id, assignment_timestamp.',
        'Exposure event: fires client-side when the user actually sees the treated UI. Includes user_id, experiment_id, variant_id, exposure_timestamp. Fire for both control and treatment.',
        'Primary metric event: confirm the event that constitutes the primary metric (e.g., checkout_completed) has been firing reliably for at least 2 weeks before the experiment starts. Check for recent schema changes.',
        'Guardrail metric events: confirm all guardrail metrics are instrumented. If you will use page_load_time as a guardrail, confirm it is tracked for the experiment surfaces.',
        'Variant property on downstream events: attach experiment_variant as a property on all conversion events during the experiment window. This enables segment-level analysis without joining to the assignment log.',
      ]},
      { type: 'heading', text: 'Sample Ratio Mismatch: The Canary in the Mine' },
      { type: 'text', text: 'Sample Ratio Mismatch (SRM) occurs when the observed ratio of users across variants differs significantly from the configured ratio. A 50/50 experiment that produces 55/45 in the data has an SRM. SRM is almost always caused by instrumentation or assignment problems, not by the treatment effect.\n\nCommon SRM causes: the assignment event fires multiple times for returning users and one variant fires more; a redirect in the treatment group drops some users before the assignment event fires; a consent banner suppresses the assignment event more in one variant.\n\nSRM does not just mean the experiment is imbalanced — it means the populations being compared are not equivalent, so all metric differences are confounded.' },
      { type: 'cta', room: 'instrumentation', label: 'Practice Instrumentation Room →' },
    ],
    keyTakeaways: [
      'Assignment ≠ exposure. Use exposed users (those who actually saw the treatment) as the denominator, not all assigned users.',
      'The exposure event must fire for both control and treatment — otherwise you have no control baseline for exposed-user analysis.',
      'Pre-experiment checklist: assignment event, exposure event, primary metric event (2+ weeks of clean history), guardrail events, variant property on conversion events.',
      'SRM (Sample Ratio Mismatch) — assigned ratio deviates significantly from configured ratio — indicates an instrumentation or assignment problem that invalidates the experiment.',
      'Common SRM causes: assignment event firing multiple times, redirect dropping users, consent banner asymmetry across variants.',
    ],
  },

  {
    id: 'take-home-structure',
    category: 'Take-Home',
    title: 'The Take-Home Analysis Framework: Structure Before SQL',
    summary: 'Analysts who open a SQL editor first rarely end up with the right answer. Structure the problem first.',
    readMin: 7,
    source: null,
    relatedItems: [
      { label: 'Take-Home Room — TH01: Instagram Stories Creator Decline', room: 'take-home', id: 'TH01' },
      { label: 'Take-Home Room — TH04: Fraud Chargeback Spike', room: 'take-home', id: 'TH04' },
    ],
    content: [
      { type: 'text', text: 'A take-home analysis assignment is not a test of SQL proficiency — it is a test of analytical judgment. The most common failure mode is jumping to queries before establishing a framework. The analyst runs 15 queries, gets a pile of numbers, and writes a summary that reads like a list of observations rather than an investigation with a conclusion. Reviewers see this pattern immediately.' },
      { type: 'heading', text: 'What Reviewers Actually Look For' },
      { type: 'list', items: [
        'Problem framing: does the analyst understand what they are actually investigating? A metric decline is a symptom — the analyst\'s job is to identify the root cause, not to describe the decline from different angles.',
        'Hypothesis-driven approach: does each analytical step test a specific hypothesis? Or does the analyst just explore until something interesting appears?',
        'Prioritized investigation: does the analyst focus on the most likely explanations first? Or do they spend equal time on low-probability hypotheses?',
        'Defensible conclusion: does the analyst reach a specific conclusion with stated confidence level? Or do they end with "it could be X, Y, or Z"?',
        'Honest limitations: does the analyst acknowledge what they cannot conclude from the available data?',
      ]},
      { type: 'callout', label: 'The analyst\'s job in a take-home', text: 'Your job is not to report what the data shows. Your job is to answer the business question the data was given to you to answer. Every finding should be in service of that answer. If a finding does not help answer the question, cut it — even if it is interesting.' },
      { type: 'heading', text: 'The Four-Step Framework' },
      { type: 'text', text: 'Before writing any SQL, write four things on paper or in a scratch doc:\n\n1. Restate the question precisely: what exactly are you trying to explain or decide? Not "Instagram Stories creator decline" but "why have weekly active Stories creators declined 12% MoM since June, and what is the most likely intervention?"\n\n2. Generate your hypotheses: list the possible explanations, categorized as external (seasonal, competitive), product (feature change, bug, UX regression), measurement (tracking break, definition change), or mix-shift (user segment composition changed). This is your investigation map.\n\n3. Rank hypotheses by prior probability: which explanations are most likely given what you already know about the product and business? Start with the most probable.\n\n4. Define what you would see in the data if each hypothesis is true: "if it\'s seasonal, YoY comparison will show the same pattern last year. If it\'s a product bug, the drop will be concentrated in a specific app version or surface."\n\nOnly after completing these four steps should you open a query editor.' },
      { type: 'heading', text: 'Structuring the Written Deliverable' },
      { type: 'text', text: 'The written output of a take-home should read like a short consulting memo, not a data exploration log. Sections:\n\nExecutive summary (2–3 sentences): the finding and the recommendation. This is what a busy PM will read.\n\nAnalysis: hypotheses tested, what the data showed for each, which were eliminated and which were supported. Show your work, but organized by hypothesis, not by query number.\n\nConclusion: the most likely root cause with stated confidence level. "The data most strongly supports [X] as the primary driver. [Y] cannot be ruled out but explains a smaller share of the observed change."\n\nLimitations: what you could not determine from the available data, and what additional data would resolve the uncertainty.\n\nRecommendation: one or two specific next steps.' },
      { type: 'cta', room: 'take-home', label: 'Practice Take-Home Room →' },
    ],
    keyTakeaways: [
      'Reviewers evaluate analytical judgment, not SQL fluency. Structure the problem before opening a query editor.',
      'Hypothesis-driven approach: every query tests a specific hypothesis. Exploratory queries that produce a pile of numbers are not an analysis.',
      'Four pre-SQL steps: restate the question precisely, list hypotheses by category, rank by prior probability, define what you would see if each is true.',
      'The written deliverable structure: executive summary → analysis (organized by hypothesis) → conclusion with confidence level → limitations → recommendation.',
      'Your job is not to report what the data shows. Your job is to answer the business question the data was given to answer.',
    ],
  },

  {
    id: 'take-home-metric-decline',
    category: 'Take-Home',
    title: 'Diagnosing a Metric Decline: The Right Order of Operations',
    summary: 'The first question is always: is the decline real? The second is: where is it concentrated?',
    readMin: 8,
    source: null,
    relatedItems: [
      { label: 'Take-Home Room — TH02: Host Supply Crunch in Urban Markets', room: 'take-home', id: 'TH02' },
      { label: 'Take-Home Room — TH05: Driver 6-Month Retention Dropped 15%', room: 'take-home', id: 'TH05' },
    ],
    content: [
      { type: 'text', text: 'Metric decline investigations are the most common take-home format. You are given a metric that dropped, access to query a dataset, and asked to find the root cause. The candidates who do this well follow a consistent order of operations. The candidates who struggle either skip to symptom description ("conversion dropped 8% across all platforms") or jump to a conclusion without systematically eliminating alternatives.' },
      { type: 'heading', text: 'Step 1: Confirm the Decline is Real' },
      { type: 'text', text: 'Before investigating root cause, confirm the decline is genuine and not a data artifact. Check:\n\nMeasurement continuity: did the metric definition or tracking implementation change around the time of the decline? A redeployment of the tracking pixel, a change in event property mapping, or a new attribution window can create an apparent decline that is entirely artificial.\n\nSeasonality: is this decline expected at this time of year? Compare to the same period last year.\n\nData freshness: is the pipeline fully loaded? A dashboard showing yesterday\'s data as a decline when the pipeline hasn\'t ingested the last 6 hours yet is an incomplete query, not a business problem.' },
      { type: 'heading', text: 'Step 2: Locate the Decline (Segmentation)' },
      { type: 'list', items: [
        'Time: when exactly did the decline start? Is it a step-change (sudden drop on a specific date) or a gradual drift?',
        'Geography: is it concentrated in specific markets or regions? A sudden supply drop in urban markets looks very different from a global trend.',
        'Platform: web vs iOS vs Android. Platform-specific drops strongly suggest a technical root cause on that platform.',
        'User segment: new users vs returning users. If only new users dropped, acquisition quality changed. If only returning users dropped, engagement or retention changed.',
        'Cohort: are all cohorts equally affected, or are newer cohorts worse? If only cohorts acquired after a specific date are declining, something changed in the acquisition flow or onboarding.',
      ]},
      { type: 'callout', label: 'Concentration is the finding', text: 'If a metric dropped 12% in aggregate but 40% in one segment and 0% in all others, the root cause is in that segment. Aggregate metrics are symptoms. Segments are diagnoses. The most valuable analysis in a metric decline take-home is often just: "the decline is entirely concentrated in [X segment], which accounts for 100% of the observed change."' },
      { type: 'heading', text: 'Step 3: Generate and Test Hypotheses' },
      { type: 'text', text: 'Once you know when and where the decline is concentrated, generate hypotheses in three categories:\n\nExternal: competitive entry, macro conditions, seasonality, regulatory change.\n\nProduct: feature change, bug, UX regression, pricing change, policy change.\n\nMeasurement: tracking break, definition change, pipeline issue.\n\nFor each hypothesis, define the data pattern that would confirm or eliminate it, then run the query. Document what you found for each, not just the final answer. Reviewers want to see the elimination process.' },
      { type: 'heading', text: 'Applied Example: Driver Retention Decline' },
      { type: 'example', label: 'Take-Home Investigation — Driver 6-Month Retention', text: 'GIVEN\n  Driver 6-month retention dropped from 68% to 53% (-15pp)\n  Time period: Q3 cohorts vs Q2 cohorts\n  Data available: driver trips, onboarding events, payout records,\n  surge acceptance, support contacts, deactivations\n\nSTEP 1: IS IT REAL?\n  Tracking check: retention definition unchanged (still: ≥1 trip in\n  month 6 of cohort). No pipeline changes in Q3. Confirmed real.\n  Seasonal: Q3 cohorts in prior year showed 66% 6M retention.\n  This year: 53%. -13pp YoY — also anomalous. Decline is real.\n\nSTEP 2: WHERE IS IT CONCENTRATED?\n  By market: urban markets -19pp, suburban -7pp, rural -2pp\n  By onboarding channel: app-store organic -8pp, referral -21pp\n  By payout tier: bottom quartile drivers -31pp, top quartile -4pp\n\n  Primary concentration: urban + referral-acquired + bottom-quartile payout\n\nSTEP 3: HYPOTHESES\n  H1: Surge pricing reduction in urban markets → reduced payout for\n      low-trip-frequency drivers → economically unviable → churn\n      Data test: payout per hour for bottom quartile drivers in urban markets\n      Result: Urban Q3 cohort bottom-quartile drivers earned $12.40/hr\n      vs $17.20/hr in Q2. SUPPORTED.\n\n  H2: Referral program quality degraded (lower-intent drivers acquired)\n      Data test: completion rate of first 10 trips\n      Result: Referral Q3 cohorts completed 6.2 of first 10 trips vs\n      8.1 in Q2 referral cohorts. SUPPORTED.\n\nCONCLUSION\n  Two compounding causes:\n  (1) Urban surge reduction cut hourly earnings below viable threshold\n      for low-frequency drivers, driving early churn\n  (2) Referral program incentive change attracted lower-commitment drivers\n  Combined, these two segments account for 71% of the observed retention drop.\n  Recommendation: restore urban surge floor OR add guaranteed minimum\n  hourly rate for drivers <30 trips/month. Revise referral incentive\n  structure to reward trip completion, not just sign-up.' },
      { type: 'cta', room: 'take-home', label: 'Practice Take-Home Room →' },
    ],
    keyTakeaways: [
      'Order of operations: confirm the decline is real (not a tracking artifact or seasonality) → locate where it is concentrated → hypothesize and test.',
      'Concentration is the finding. A 12% aggregate drop that is 40% in one segment and 0% elsewhere has its root cause in that segment.',
      'Segmentation dimensions: time (step-change vs gradual), geography, platform, user segment (new vs returning), cohort.',
      'For each hypothesis, define the data pattern that would confirm it before running the query. Document eliminations, not just the final answer.',
      'In the driver example: two compounding causes (urban surge reduction + referral program quality) explained 71% of the 15pp retention drop.',
    ],
  },

  {
    id: 'take-home-cohort-analysis',
    category: 'Take-Home',
    title: 'Cohort Analysis in Take-Homes: When to Slice and What to Look For',
    summary: 'Cohort analysis is the right tool for retention and engagement problems. It\'s the wrong tool for reach problems.',
    readMin: 6,
    source: null,
    relatedItems: [
      { label: 'Take-Home Room — TH03: Podcast Listeners Convert at Half the Rate', room: 'take-home', id: 'TH03' },
      { label: 'Take-Home Room — TH05: Driver 6-Month Retention Dropped 15%', room: 'take-home', id: 'TH05' },
    ],
    content: [
      { type: 'text', text: 'Cohort analysis groups users by a shared characteristic — most commonly, when they were acquired — and tracks their behavior over time. It is one of the most powerful tools in a product analyst\'s toolkit, but it is frequently misapplied in take-home assignments. Knowing when to use it and what patterns to look for separates candidates who reach correct conclusions from those who produce technically correct but analytically useless tables.' },
      { type: 'heading', text: 'When Cohort Analysis is the Right Tool' },
      { type: 'list', items: [
        'Retention problems: if you are asked why engagement or return rate is declining, cohort analysis shows whether the decline is in all cohorts (product regression) or just newer cohorts (acquisition quality or onboarding change).',
        'Onboarding funnel problems: if you are asked why new users are converting or activating at a lower rate, acquisition-date cohorts show when the change started, which timestamps it to a specific product change.',
        'Lifecycle revenue problems: LTV and cohort-level revenue curves reveal whether monetization is improving or declining over a user\'s lifetime, not just in aggregate.',
      ]},
      { type: 'heading', text: 'When Cohort Analysis is the Wrong Tool' },
      { type: 'text', text: 'Cohort analysis is not useful for top-of-funnel questions where the denominator changes by acquisition date for reasons unrelated to the product. If podcast listener acquisition doubled in Q3 due to a podcast advertising campaign, a Q3 cohort will look worse than a Q2 cohort on conversion rate — not because the product got worse, but because the Q3 cohort has lower intent (they came from a broad awareness channel).\n\nThis is the mix-shift problem. Cohort divergence does not always mean the product changed — it can mean the acquired population changed. Always check acquisition channel composition when cohort performance diverges.' },
      { type: 'callout', label: 'The cohort divergence test', text: 'When you see newer cohorts performing worse than older cohorts, ask two questions before concluding the product regressed: (1) Did the acquisition channel mix change when the divergence started? (2) Did any onboarding or early-experience change go out around that date? If channel mix changed significantly, the cohort divergence may be a mix-shift artifact, not a product signal.' },
      { type: 'heading', text: 'Two Types of Cohort: Time-of-Acquisition vs Behavioral' },
      { type: 'text', text: 'Most analysts default to time-of-acquisition cohorts (January cohort, Q1 cohort). But in take-home assignments involving specific user segments, behavioral cohorts are often more revealing:\n\nBehavioral cohort example: users who listened to ≥3 podcast episodes in their first week vs those who listened to ≤1. If the first group converts to premium at 4× the rate of the second, the analysis should focus on early engagement depth, not acquisition timing.\n\nFor the podcast listeners take-home: if podcast listener conversion is 50% of direct visitor conversion, the question is why. A behavioral cohort by early engagement depth (episode completions in first week) almost certainly shows that conversion correlates strongly with early engagement — which points to the recommendation: optimize the first-week content experience, not the paywall conversion flow.' },
      { type: 'cta', room: 'take-home', label: 'Practice Take-Home Room →' },
    ],
    keyTakeaways: [
      'Cohort analysis is best for retention, onboarding funnel, and lifecycle revenue problems — not for top-of-funnel reach or acquisition questions.',
      'Cohort divergence (newer cohorts performing worse) has two possible causes: product regression OR acquisition mix shift. Check both before concluding.',
      'Mix-shift check: did acquisition channel composition change when the cohort divergence started? Different channels bring users with different intent levels.',
      'Behavioral cohorts (grouped by early engagement depth) often reveal more than time-of-acquisition cohorts for conversion and activation problems.',
      'For podcast listeners: behavioral cohort by episode completions in week 1 likely shows conversion correlates with early engagement — pointing to experience optimization, not paywall optimization.',
    ],
  },

  {
    id: 'take-home-uncertainty',
    category: 'Take-Home',
    title: 'Communicating Uncertainty in Data Analyses: Confidence Without Overclaiming',
    summary: 'Hedging every finding with "it could be X or Y" reads as weak. Overclaiming reads as sloppy. The answer is calibrated precision.',
    readMin: 6,
    source: null,
    relatedItems: [
      { label: 'Take-Home Room — TH04: Fraud Chargeback Spike', room: 'take-home', id: 'TH04' },
    ...
    content: [
      { type: 'text', text: 'Data analysis almost always produces findings under uncertainty. You have partial data, imperfect proxies, confounding you could not control for. The question is how to communicate that uncertainty without either burying the finding or overclaiming certainty you do not have. Both errors damage analytical credibility — but in opposite ways.' },
      { type: 'heading', text: 'The Two Failure Modes' },
      { type: 'text', text: 'Overclaiming: "The fraud spike was caused by a compromised merchant partner account." The data showed a spike concentrated in one merchant\'s transactions. That is evidence consistent with this hypothesis — but you cannot confirm it without access to payment processor logs and fraud investigation data that are not in the take-home dataset. Presenting a hypothesis as a conclusion makes you look analytically immature.\n\nUnderclaiming: "There are many possible causes of the fraud spike, including compromised accounts, bot activity, stolen card numbers, unusual merchant behavior, and possibly seasonal effects, though it is difficult to determine which is most likely from the available data." This is a non-answer. You just described the problem without advancing toward a solution. Reviewers see this as analytical passivity.' },
      { type: 'heading', text: 'The Confidence Ladder' },
      { type: 'list', items: [
        'Observed (highest confidence): what the data directly shows. "Fraud chargebacks are concentrated in transactions from Merchant ID 7823, which accounts for 71% of the spike." This is a fact.',
        'Inferred (medium confidence): what the observed pattern implies, given domain knowledge. "This concentration pattern is consistent with a compromised merchant partner account or merchant-side fraud. These two hypotheses are the most common causes of this type of spike in payment systems."',
        'Assumed (lower confidence): conclusions that require data you do not have. "The most likely cause is a compromised merchant account." Note this explicitly: "I cannot confirm this without payment processor transaction logs — this is an inference from pattern, not a confirmed root cause."',
      ]},
      { type: 'callout', label: 'The calibrated conclusion template', text: '"The data most strongly supports [X] as the primary driver (observed: [specific finding]). [Y] cannot be ruled out but would require [Z data] to confirm. If [X] is correct, the recommended action is [specific action]." This structure gives a clear answer, shows your reasoning, acknowledges the uncertainty honestly, and provides a path forward.' },
      { type: 'heading', text: 'What to Put in the Limitations Section' },
      { type: 'text', text: 'A limitations section is not a defensive disclaimer — it is an honest accounting of what additional data would change or strengthen your conclusion. Good limitations statements:\n\n"I cannot determine whether the fraud is merchant-initiated or cardholder-side without access to chargeback dispute codes, which are not in this dataset."\n\n"The 6-week analysis window may be insufficient to determine whether this is a trend or a one-time anomaly — 6 months of history would improve confidence in the trend classification."\n\nWeak limitations statements: "The data may have quality issues." "I did not have access to all relevant data." These are generic and add no analytical value. If you are going to note a limitation, specify exactly what data would resolve it and how it would change your conclusion.' },
      { type: 'cta', room: 'take-home', label: 'Practice Take-Home Room →' },
    ],
    keyTakeaways: [
      'Two failure modes: overclaiming (presenting a hypothesis as a confirmed conclusion) and underclaiming (listing possibilities without recommending one). Both damage credibility.',
      'The confidence ladder: Observed (what data directly shows) → Inferred (what it implies given domain knowledge) → Assumed (requires data you don\'t have).',
      'Calibrated conclusion template: "Data most strongly supports X (observed: [finding]). Y cannot be ruled out but requires [data]. If X is correct, recommended action is [action]."',
      'Limitations section: specify what data would resolve the uncertainty and how it would change your conclusion. Generic disclaimers add no value.',
      'Reaching a specific conclusion with stated confidence is a sign of analytical maturity — not overclaiming. Reviewers want a defensible answer, not a list of possibilities.',
    ],
  },
...
const ROOM_CONFIG = {
  ...
  bi:                 { label: 'BI Room',                 color: 'var(--yellow)',    page: 'bi',               openFn: 'openBICase' },
  instrumentation:    { label: 'Instrumentation Room',   color: 'var(--teal)',      page: 'instrumentation',  openFn: 'openInstrumentationCase' },
    ...
    <div className="pal-page-enter" style={{ maxWidth: '1040px', margin: '0 auto', padding: '1.75rem 1rem 3rem' }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.5rem' }}>
          ...
          <h1 style={{ fontSize: '1.45rem', fontWeight: 900, color: 'var(--text)', margin: 0, letterSpacing: '-0.02em' }}>Frameworks</h1>
        </div>
