...
function ensureStyles() {
  ...
  s.id = 'pal-hl-native-styles'
  s.textContent = Object.keys(FAINT)
    .map(function (k) { return '::highlight(pal-hl-' + k + ') { background-color: ' + FAINT[k] + '; }' })
    .join('\n')
...
function rebuildRegistry() {
  ...
  for (const name of Object.keys(FAINT)) {
    ...
    if (ranges && ranges.length) CSS.highlights.set('pal-hl-' + name, new Highlight(...ranges))
    else CSS.highlights.delete('pal-hl-' + name)
  }
}
