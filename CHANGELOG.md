...
- **Scaled across the WHOLE eligible bank** (13 subagent batches — 6 Hard/Master + 7 Medium — each independently gate-verified): **106 problems now carry the layer, 336 methods, all harness-verified** (non-traps byte-match `solution`, traps run+diverge). Every Hard, Master, and Medium problem is covered; Easy (single-method fluency by design) and Forensic (already the judgment format) are intentionally excluded. Spans top-N-per-group, rank-within-group, median/percentile, quantile-bucketing, gaps-and-islands, sessionization, recursion, anti-join, relational division, cohort/retention, ratio/share, MoM, running/rolling windows, interval-overlap, set-ops, string-parse, pivot. The honesty rule (spike §3.3) held throughout: **7 genuine single-method problems got an empty dial** (`master01`/`master04`/`master10` composite scorers, `h10` merchant exposure, `m25`/`m43`, `m78` debug) rather than a faked fork — the empty dial is itself the "this is fluency, not judgment" signal. Several batches also honestly authored valid-method-only sets (no faked trap) where no query diverged on the seed (e.g. `h39`).
- **Side benefit — ~25 stale debriefs fixed.** Authoring each method against the live seed surfaced legacy debrief prose whose "wrong answer"/forensic-trap claims no longer diverged on the current data (across `h02/h07/h17/h24/h10/h32/h14/h39/h25`, many `master*`, and `m01/m07/m17/m21/m24/m25/m32/m33/m37/m42/m43/m47/m56/m57/m61/m77/m78` + `str1`). Each stale claim was rewritten to the genuinely-divergent trap with verified numbers; the methods were always correct, only the prose had drifted — so this pass doubled as a real content-accuracy cleanup.
...
Gates green: harness 106/106 pass (336 methods, 0 failures), audit 0 T1 (count unchanged at 192), content scan 0 flags, brace 0; `SqlLabPage.jsx` parses clean. Files: `scripts/verify_methods.py` (new), `src/data/sqlLabProblems.js`, `src/pages/SqlLabPage.jsx`. **Propose-only — not pushed.**
